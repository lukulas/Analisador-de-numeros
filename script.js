...
function adicionar(){
    if(isNumero(num.value) && !inLista(num.value, valores)){
        ...
        window.alert('[ERRO] O numero digitado não é aceitavel')
    }
    num.value = ''
    num.focus()
}
function finalizar(){
    if(valores.length == 0){
        ...
        res.innerHTML += `<p> O maior valores informado foi o ${maior}. </p>`
        res.innerHTML += `<p> O menor valores informado foi o ${menor}. </p>`
